...
validate_env_set() {
    (
        ...
        if [ -z "${!1}" ]; then
            echo "Variable '$1' was not set. Aborting"
            exit 1
        fi
    )
}
...
if [ "$KUBERNETES_VERSION" == "1.17.17" ]; then
    KUBERNETES_BUILD_DATE="2021-09-02"
elif [ "$KUBERNETES_VERSION" == "1.18.20" ]; then
    KUBERNETES_BUILD_DATE="2021-09-02"
elif [ "$KUBERNETES_VERSION" == "1.19.14" ]; then
    KUBERNETES_BUILD_DATE="2021-10-12"
elif [ "$KUBERNETES_VERSION" == "1.20.10" ]; then
    KUBERNETES_BUILD_DATE="2021-10-12"
elif [ "$KUBERNETES_VERSION" == "1.21.4" ]; then
    KUBERNETES_BUILD_DATE="2021-10-12"
else
    echo "Unknown kubernetes version '$KUBERNETES_VERSION'" >&2
    exit 1
fi
